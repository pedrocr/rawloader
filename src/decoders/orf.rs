...
impl<'a> OrfDecoder<'a> {
      ...
      for c in 0..width/2 {
        ...
        for s in 0..2 { // Run twice for odd and even pixels
          ...
          if row < 2 || col < 2 { // We're in a border, special care is needed
            ...
            } else if left_minus_nw.abs() > up_minus_nw.abs() { left[s] } else { up };
  ...
  fn get_wb(&self) -> Result<[f32;4],String> {
    ...
    if let (Some(redmul_entry), Some(bluemul_entry)) = (redmul, bluemul) {
      Ok([redmul_entry.get_u32(0) as f32,256.0,bluemul_entry.get_u32(0) as f32,NAN])
    } else {
